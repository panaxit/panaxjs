...
exports.parseResults = function(xml, callback) {
	...
	xmlDoc.root().childNodes().forEach(function (resultNode) {
		var result_attrs = resultNode.attrs();
		var result = {};
		for(var i=0;i<result_attrs.length;i++) {
			result[result_attrs[i].name()] = result_attrs[i].value();
		}
		result.fields = [];
		resultNode.childNodes().forEach(function (fieldNode) {
			var field_attrs = fieldNode.attrs();
			var field = {};
			for(var i=0;i<field_attrs.length;i++) {
				field[field_attrs[i].name()] = field_attrs[i].value();
			}
			if(fieldNode.text()) {
